...
v0atomop(struct v0 *vm, struct v0ins *ins)
{
    ...
    if (ins->args == V0_WR_BIT) {
        ...
        if (!ins->args) {
            ...
            if (ins->args & V0_CLR_BIT) {
                val &= ~bit;
            } else {
                val |= bit;
            }
            v0putmem(vm, adr, val, int32_t, 0xffffffff);
        } else if (ins->args & V0_SYN_BIT) {
            bit = 1L << arg1;
            if (ins->args & V0_CLR_BIT) {
                val &= ~bit;
            } else {
...
v0flowop(struct v0 *vm, struct v0ins *ins)
{
    ...
    pc = m_atomadd((m_atomic_t *)&vm->regs[VM_PC_REG], (m_atomic_t)incr);
    pc += incr;
    if (!ins->args || ins->args == V0_REL_BIT) {
        pc = argu1;
    } else if (ins->args == V0_EQ_BIT) {
        if (vm->regs[VM_MSW_REQ] & V0_ZF_BIT) {
            pc = argu1;
        }
    } else if (ins->args & V0_NE_BIT) {
        if (!(vm->regs[VM_MSW_REQ] & V0_ZF_BIT)) {
            pc = argu1;
        }
    } else if (ins->args & V0_LT_BIT) {
        if (!(vm->regs[VM_MSW_REG] & V0_ZF_BIT)
            && !(vm->regs[VM_MSW_REG] & V0_CF_BIT)) {
            pc = argu1;
        }
    } else if (ins->args & V0_GT_BIT) {
        if (!(vm->regs[VM_MSW_REG] & V0_ZF_BIT)
            && (vm->regs[VM_MSW_REG] & V0_CF_BIT)) {
            pc = argu1;
        }
    } else if (ins->args & V0_BFL_BIT) {
        if (ins->args & V0_BOF_BIT) {
